    ...
    class BankSystem
    {
        ...
        static void DoRollback(Bank bank)
        {
           bank.PrintTransactionHistory();
            Console.WriteLine("Plz enter transaction # rollback (0 for no rollback");
            int result = Convert.ToInt32(Console.ReadLine());
            while (result < 0 || result > bank.Transactions.Count)
            {
                Console.WriteLine("Plz enter whole number from correct range ");
            }
            if (result == 0)
            {
                return;
